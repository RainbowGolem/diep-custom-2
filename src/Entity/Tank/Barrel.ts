...
import TankDefinitions, { BarrelDefinition, TankCount, TankDefinition } from "../../Const/TankDefinitions";
...
import barrelsAmmount from "./TankBody";
/**
...
let reloadingTime: number;

let shotsBeforeReload: number;
...
export class ShootCycle {
    ...
    /** The ammount of shots that the tank can shoot before the reload... */
    private shotsBeforeReload: number;
    /** The time that it takes for the tank to reload... */
    private reloadingTime: number;
    ...
    public constructor(barrel: Barrel) {
        ...
        this.shotsBeforeReload = this.barrelEntity.definition.shotsBeforeReload
        // console.log(this.shotsBeforeReload, 'this.shots before reload');

        this.reloadingTime = this.barrelEntity.definition.reloadingTime;
        // console.log(this.reloadingTime, 'this.reloadingTime');
        reloadingTime = this.reloadingTime * this.barrelEntity.definition.barrelsAmmount;
        // console.log(reloadingTime, 'let reloading time');

        shotsBeforeReload = this.shotsBeforeReload;
        // console.log(shotsBeforeReload, 'shots before reload')

        // console.log(this.barrelEntity.definition.barrelsAmmount);
    }
...
export default class Barrel extends ObjectEntity {
    ...
    public shoot() {
        // console.log(shotsBeforeReload, 'shot', reloadingTime, 'reloading time')


        reloadingTime = this.definition.reloadingTime * this.definition.barrelsAmmount;

        if (shotsBeforeReload > 0 && isReloading === false) {
            this.barrelData.flags ^= BarrelFlags.hasShot;

            shotsBeforeReload--;
            // console.log('shots --');
            // console.log(reloadingTime);
        ...
        } else if (shotsBeforeReload === 0) {
            isReloading = true;
            // console.log(isReloading, 'is reloading')
        }
    }
    ...
    public tick(tick: number) {
        this.resize();
        if (isReloading === true && reloadingTime > 0) {
            
            reloadingTime--;
            // console.log(reloadingTime, 'reloading time');
            ...
            isReloading = false;
            reloadingTime = this.definition.reloadingTime * this.definition.barrelsAmmount;
            // console.log(reloadingTime, 'is reloading false');
            shotsBeforeReload = this.definition.shotsBeforeReload;
        }
    }
}
